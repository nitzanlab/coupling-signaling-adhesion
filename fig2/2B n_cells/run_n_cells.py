...
n_values = np.arange(1,10)*100
# n_values = [250,500]

# Function to start the simulation process with a delay between launches
async def run_simulation(repeat, is_const,n_cells):
    start_time = time.time()
    print(f"Starting simulation with repeat {repeat + 1}")
    if is_const:
        process = await asyncio.create_subprocess_exec(
            "python", "cell_merging_simulation_no_graphics.py", '--k', '0', '--n',str(n_cells),
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
    else:
        process = await asyncio.create_subprocess_exec(
            "python", "cell_merging_simulation_no_graphics.py", '--n',str(n_cells),
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
