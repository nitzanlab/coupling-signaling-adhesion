...
def run_single_simulation(args):
    temp, rep, is_induced = args
    return {
        'T': temp,
        'rep': rep,
        'experiment': 'induced' if is_induced else 'constitutive',
        'n_cores': run_simulation(IS_INDUCED=is_induced, T=temp)
    }

# Define the main function for parallel execution
def run_parallel_simulations(n_reps, temps, n_workers=4):
    # Create argument list for all combinations
    args_list = [
        (temp, rep, is_induced)
        for temp in temps
        for rep in range(1, n_reps + 1)
        for is_induced in [True, False]
    ]
    ...
    with open(f'{filename}.pkl', 'wb') as f:
        pickle.dump({'df': results_df}, f)

    print(f"Simulation complete. Results saved to {filename}.pkl")
    return results_df

# Define temperatures and repetitions
n_reps = 100
temps = [0.05, 0.25, 0.5, 2.5, 5, 25, 50, 250, 500, 2500]

# Run the simulations in parallel

# Main execution block
if __name__ == '__main__':
    # Ensure the script is protected with this block
    results_df = run_parallel_simulations(n_reps, temps, n_workers=multiprocessing.cpu_count())
